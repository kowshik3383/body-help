...
export default function BodyPartPage({ params }: BodyPartPageProps) {
  ...
  useEffect(() => {
    ...
    const fetchTreatments = async () => {
      try {
        ...
        const response = await fetch(`/api/treatments?disease=${encodeURIComponent(selectedDisease.name)}&language=${encodeURIComponent(content.code)}`);
  ...
  return (
    <div className="relative font-poppins w-full h-screen bg-gradient-to-br from-blue-50/50 via-white to-purple-50/50 dark:from-gray-950 dark:via-gray-900 dark:to-blue-950/30">
        ...
        <div className="flex flex-col lg:flex-row h-full">
          ...
          <div className="w-full lg:flex-1 relative flex flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-blue-50/30 to-purple-50/30 dark:from-gray-900/50 dark:to-blue-950/30 border-b lg:border-b-0 lg:border-r border-gray-200/50 dark:border-gray-800/50 h-[35vh] sm:h-[40vh] lg:h-full">
            ...
            <motion.div
              ...
              className="absolute top-2 sm:top-3 lg:top-4 left-2 sm:left-3 lg:left-4 z-10 flex gap-2"
            >
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              ...
              <motion.button
                ...
                className="px-2.5 py-1.5 sm:px-3 sm:py-2 lg:px-4 lg:py-2.5 bg-gradient-to-r from-gray-700 to-gray-900 hover:from-gray-800 hover:to-black text-white rounded-lg sm:rounded-xl transition-all shadow-lg hover:shadow-xl text-xs sm:text-sm font-medium flex items-center gap-1.5 sm:gap-2 backdrop-blur-sm"
              >
                <span className="hidden xs:inline">3D Model</span>
              ...
              >
                {showModel && bodyPartData ? (
                  ...
                  <div className="relative h-screen w-full max-w-md">
                    ...
                    {/* Map is now in modal; keep placeholder space here if needed */}
                  </div>
                )}
              </motion.div>
            </AnimatePresence>
                                ...
                                {diseases.map((disease, idx) => (
                                  ...
                                  >
                                    ...
                                    <div className="relative">
                                      <h3 className="font-bold text-gray-900 dark:text-white mb-1.5 sm:mb-2 text-md sm:text-sm lg:text-base flex items-center gap-1.5 sm:gap-2">
      ...
      {/* Human Map Modal */}
      {isMapOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setIsMapOpen(false)}>
          <div className="relative w-[95vw] h-[90vh] max-w-5xl bg-white dark:bg-gray-950 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <button
              onClick={() => setIsMapOpen(false)}
              className="absolute top-3 right-3 z-10 px-3 py-1.5 rounded-lg bg-gray-900 text-white text-xs"
            >
              Close
            </button>
